...
export default function App() {
  ...
  const handleImport = (newPairs: LetterPair[], url?: string) => {
    // Buat map dari data baru (dari spreadsheet) untuk pencarian cepat
    const newItemsMap = new Map(newPairs.map((p) => [p.id, p]));

    // Lakukan penggabungan (merge) data yang sudah ada
    const mergedPairs = normalizedPairs.map((existingPair) => {
  ...
  return (
    <div className="min-h-[100svh] overflow-hidden bg-bg-base text-text-primary font-sans flex flex-col md:flex-row">
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:static w-full md:w-64 glass-nav md:glass border-t border-t-white/5 md:border-t-0 md:border-r border-r-white/5 p-4 md:p-6 flex flex-row md:flex-col shrink-0 order-last md:order-first z-50 pb-safe md:pb-6">
      ...
      {/* Main Content Area */}
      <main
        className={`flex-1 px-4 py-2 md:p-12 pb-24 md:pb-12 ${
          activeTab === "study-corner" || activeTab === "study-edge"
            ? "overflow-hidden"
            : "overflow-y-auto"
        }`}
      >
