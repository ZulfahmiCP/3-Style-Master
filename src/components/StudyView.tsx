...
export function StudyView({ pairs, onUpdatePair, onNavigateToImport }: StudyViewProps) {
  ...
  if (pairs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-md mx-auto space-y-6 animate-in zoom-in-95 duration-500">
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-md mx-auto space-y-6 animate-in zoom-in-95 duration-500">
          ...
          <div>
            ...
            <p className="text-text-muted mt-2 text-sm leading-relaxed">
              You've studied all non-mastered letter pairs in your currently imported data. 
              Reset some pairs in the library or import more to continue.
            </p>
          </div>
        </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col items-center min-h-[60vh] justify-center">
      ...
      {/* The Flashcard */}
      <div 
        className="relative w-full aspect-[4/3] md:aspect-[16/10] cursor-pointer perspective-1000 mb-8"
      ...
      >
        ...
        >
          {/* Front */}
          <div className={`absolute inset-0 backface-hidden w-full h-full card-gradient rounded-3xl flex flex-col items-center justify-center p-6 md:p-8 shadow-2xl transition-opacity duration-300 ${isFlipped ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
             <div className="absolute top-4 left-4 md:top-6 md:left-6 text-xl md:text-2xl font-bold text-white/30 flex items-center gap-2">
               <span className="text-xs font-normal opacity-50 uppercase tracking-widest">{currentPair.type || 'corner'}</span>
             </div>
          ...
          {/* Back */}
          <div className={`absolute inset-0 backface-hidden w-full h-full card-gradient rounded-3xl flex flex-col items-center justify-center p-6 md:p-8 shadow-2xl [transform:rotateX(180deg)] transition-opacity duration-300 ${!isFlipped ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
             <div className="absolute top-4 left-4 md:top-6 md:left-6 text-xl md:text-2xl font-bold text-white/30 flex items-center gap-2">
               {currentPair.letters}
               <span className="text-xs font-normal opacity-50 uppercase tracking-widest">{currentPair.type || 'corner'}</span>
             </div>
             <div className="text-center space-y-4 md:space-y-6 max-w-lg w-full mt-4 md:mt-0">
               {(currentPair.type === 'corner' || currentPair.word) && (
                 <>
                   <div>
                      <span className="text-accent text-[10px] md:text-xs font-semibold uppercase tracking-wider mb-1 md:mb-2 block">Word</span>
                      <p className="text-3xl md:text-5xl font-light text-white">{currentPair.word}</p>
                   </div>
                   <div className="w-12 h-px border-t border-white/10 mx-auto my-2 md:my-0"></div>
                 </>
               )}
               <div>
                  <span className="text-accent text-[10px] md:text-xs font-semibold uppercase tracking-wider mb-2 md:mb-4 block">Algorithm</span>
                  <code className="bg-white/5 border border-white/10 shadow-inner px-3 py-2 md:px-4 md:py-3 rounded-xl text-base md:text-xl font-mono text-text-muted break-words block">{currentPair.alg}</code>
               </div>
             </div>
          </div>
        </motion.div>
